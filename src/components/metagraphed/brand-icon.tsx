...
/**
 * Validate untrusted image sources before assigning them to <img src>.
 * Provider/registry metadata can be attacker-controlled, so only public
 * HTTP(S) URLs are eligible for browser-side loading.
 */
...
function buildCandidateChain({
  ...
  const push = (u: string | null | undefined) => {
    const safeUrl = safeImageUrl(u);
    if (!safeUrl) return;
    if (failedUrls.has(safeUrl)) return;
    if (!out.includes(safeUrl)) out.push(safeUrl);
  };
